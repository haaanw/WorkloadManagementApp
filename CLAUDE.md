...
CLAUDE is the sole committer in this repo; CLAUDE builds with
`-derivedDataPath ~/.tonus-dd-claude`; `tuwa-website/` belongs to CODEX; no
outward-facing actions (push, App Store Connect, publishing) without HAN's
explicit go.
...
- **v1.6 "Ink & Grain" (UI polish + rehost): Machine-complete 2026-07-18** — SwiftUI tree is the live app, DESIGN.md v3 applied app-wide, custom InkTabBar, full suite green. Awaiting on-device dogfood → version bump → shell deletion.
- **v1.7 "Field Notes" (v6 design system): IN PROGRESS 2026-07-30** — on branch `v1.7-field-notes` (NOT `main` — v1.6 build 17 is in App Store review and `main` stays hotfix-clean). Multi-session rollout; plan + lane briefs in `.planning/v17-field-notes/DISTRIBUTION.md`. Canonical design system: `design-system/` (in-repo, versioned).
...
- **v1.7.3 onboarding + monetization model: BUILT 2026-09-06 (Lane B, 7 commits 3e6bdb8→3fa44df) and switched ON 2026-09-14 (`OnboardingV2Flag.defaultValue = true`, HAN's go; a stored `false` under `flag.onboardingV2` is the kill switch). BINDING before submission: ASC privacy label reconciled + PostHog disclosed; `PostHogSettings.apiKey` is committed EMPTY (sink inert until HAN fills it); RevenueCat `onboarding`/`onboarding_exit` offerings absent → soft wall, no exit offer. Spec history (HAN 2026-08-24; label resolved to v1.7.3 2026-09-02)** — the paywall model itself changes. Onboarding becomes ≤12 screens ending in a **hard paywall placed after the user sees a real readiness number**, sold on Apple's **card-gated** introductory trial (a no-card trial was considered and REJECTED — it needs a server-side grant beside `SubscriptionService.isPro` and a deliberate day-7 return). The gate is **conditional on the HealthKit grant**: granted-with-data → hard paywall; denied or insufficient history → dismissible paywall, user lands in the free tier, re-ask at day 7. Account creation moves from screen 0 to screen 10; language selection leaves onboarding for Profile. The day-0 reveal is real, not theatre — the app already pulls 90 days of HealthKit history and `BaselineEngine` hits its confidence floor at 14 days. Product analytics = **PostHog**, wired *behind* `UXAnalyticsService` so its existing forbidden-fragment sanitizer stays the single egress path; bucketed values only, never a health number. Spec: `.planning/v18/ONBOARDING.md`; instrumentation, offer surfaces, ASO and the gated ad sequence: `.planning/v18/GROWTH-STACK.md`. Version label RESOLVED: ships as v1.7.3 (HAN 2026-09-02). AMENDED same day, recorded in `.planning/v18/BUILD-PLAN.md` §Amendments (overrides the spec's screen table): the “bring your program” import moment and a daily-use tutorial (led by the logging workflow; frames wait on the plan-led lane's lock) JOIN the flow; screens 6–7 (training frequency, experience level) LEAVE it — both engine-dead, editable in Profile; the `AppRouter.swift:103` completion sentinel keys on those two fields and must be re-keyed; quiz screens 2/4/5 stay; ≤12-screen cap stands.
- **v1.7.3 guided session mode (feature 9): BUILT 2026-09-08** — the execution surface after "Start this session". A MODE of `ActiveWorkoutSheet` active ONLY on its `resolvedPlan` init (template / blank / voice-parsed sessions keep the ledger): `GuidedSessionEngine` (pure set-slot queue over the sheet's `entries`; supersets = 2–3 consecutive entries sharing a `groupName` that do not span the whole session, interleaved set by set — a program day's single day-named group never interleaves) + `GuidedSessionView` (HAN-gated demo round 3 "Plate": stat wells MOVE · ELAPSED · SINCE SET, one move per hero plate on the strain wash, 44pt set blocks, two readout wells over the scrub rule via `SetEntryFields(layout: .wells)`, **Log set = the screen's one ink pill**, Skip move | + Extra set cells, next block, docked speak bar; a move ends when its last planned set logs or on Skip — no Done button). `SetDraft.loggedAt` makes `SetRecord.completedAt` honest per set (every set used to stamp the Finish moment). **Lock-screen Live Activity, tier 1** (`GuidedSessionActivityAttributes` shared app+extension, composite-only and fenced — NEVER a body number on the lock screen; `GuidedSessionLiveActivity` in `TuwaWidgets/`; `NSSupportsLiveActivities`); ActivityKit timers tick without updates; the app pushes state only on log / skip / advance / finish. Visual check of the 160pt card and the Dynamic Island is HAN's device pass. Record: `.design-explorations/session-mode-demo/PLAN.md`. **Non-strength days (round 3 · U26, 2026-09-14):** `SessionCapEngine` (pure, beside `AutoregulationEngine`) caps a planned skill / cardio / match / recovery session — max RPE = the matrix's existing `intensityCap` floored by the plan's RPE; duration = a percentage of the PLANNED duration (strain elevated 85 % · fatigue elevated 80 % · strain high 70 % · fatigue high 60 % · match ≤2 days 50 % with intensity kept · saturation ≤30 min hold; strongest wins; never exceeds the plan; no planned duration ⇒ no cap invented). The cap is recomputed each refresh and lives in memory on `ResolvedSessionPlan.sessionCap`; the DECISION persists on the existing local `TemplateSet` verdict slots — no synced schema changed. The guided session renders a PLANNED / TODAY duration readout with the RPE ceiling instead of set slots for such entries; no `VerdictEvent` is logged for a cap (kg-shaped schema). Copy law: every cap names THEIR plan ("Your 90-minute run, capped at 60 today · RPE 7"), fenced. Open follow-ons (HAN rulings 2026-09-07): the program parser gains a pair field + an import-standards moment with a copyable agent prompt; an intelligence-layer analysis that RECOMMENDS supersets; tier 2 (a `LiveActivityIntent` Log set on the lock screen); the mode's first-use tutorial.
...
- **The Area Tint (v6.3, HAN locked 2026-09-06 — "warm and whisper")** — the ONE sanctioned exception to "a hue never dresses a surface": each functional area takes a whisper of its owning hue. **Two formulas, no others**, and both live in `ColorTokens` — hero-plane wash `color-mix(in srgb, areaHue 4%, plane)`, hairline tint `color-mix(in srgb, areaHue 18%, divider)`; mirrored in `design-system/tokens/colors.css`. Never a hand-mixed color at a call site. **Area ownership:** Home/Today → readiness; Trends recovery surfaces → recovery; sleep detail → sleep; Log/capture → strain; load surfaces → load; widgets take their reading's area. A surface with **no metric identity** (Profile, settings, auth, onboarding) stays untinted stone. Applied via `.metricArea(_:)` + `.raised(isHero:)` / `.cardStyle(isHero:)` / `.heroPlane()` / `AreaRule` / `RowSeparator` / `RuledSectionHeader` — never at a call site. **REJECTED at the same gate and equally binding:** the cooled stone ramp (the warm ladder stands), section-marker squares, the area hue on the active tab tick (travertine keeps the live-state monopoly), card washes beyond the hero plane, any intensity above 4%/18%
...
- **Design**: Must follow DESIGN.md v6 "Field Notes" (overlay on v5 Pavilion; canonical source `design-system/`) — CornerTokens (card 12 / control 8 / pill), no shadows (relief via `.raised`/`.debossed`), two-voice type (Instrument Sans speaks / Fragment Mono annotates at ≤12pt), five metric hues with the hero reading in its metric's hue, travertine accent for live-state only, the v6.3 area tint (4% hero wash / 18% hairline, `ColorTokens` only), 8pt grid, light-only
## Technology Stack
...
- Responsibilities: The "Today" surface (v1.7.3 feature 7 slice 2): check-in prompt → fatigue caution → the day's proposal (`TodayProposalSection` hosts the verdict card + decision seam; no-plan state offers Bring your program / Start unplanned) → readiness hero as evidence → load/metrics/summary/recent sessions. `TodayProposalSection` is THREE-WAY (UAT round 2 · U14): verdict card when a designation exists · a quiet rest-day card naming the next planned session when a program is active but today has no entry · the bring-your-program card ONLY when no program is active. The three body-signal cells in `MetricsStrip` are TAP-THROUGHS (UAT round 1 · U9) — each pushes its own detail screen via `TrendDestination` (`.hrv` / `.rhr` / `.sleep`), which is what let Trends stop re-plotting the same three lines. **UAT round 3 (2026-09-14):** the `FatigueAttentionBanner` is UNMOUNTED from Today (component kept for Trends); the proposal's one ink pill is **Start today** → `PreSessionBriefView` (what today looks like — readings only · so today — verdict + reason with Accept / Keep-as-planned equal cells · your numbers — planned → adjusted, or the session cap) → `ActiveWorkoutSheet(resolvedPlan:)`. Today has system pull-to-refresh (`DashboardViewModel.refreshAll`: watch import → pipeline → push → pull, awaited in order) and re-runs the pipeline on `recoverySignalsChanged` (an `HKObserverQuery` over HRV / RHR / sleep registered in `AppContainer`, foreground-only, debounced). The morning sheet is the wellness ratings ONLY (HAN ruling 2026-09-15: the blinded 1–10 probe is REMOVED from the product — its `MorningReadinessProbe` model, the `RecoveryShadowDay` outcome columns and the no-engine-reads-it fence stay; the VALIDATION-PROTOCOL's primary held-out outcome is therefore no longer collected and the estimator-v2 flip needs another outcome or the probe re-enabled).
- Location: `WorkloadApp/Views/Trends/TrendsView.swift`
- Responsibilities: The over-time surface, re-scoped around the FATIGUE NARRATIVE (v1.7.3 · UAT round 1 · U9): range rail (1W / 2W / 1M, Pro; free reads the 2W default) → fatigue hero (`FatigueHistoryEngine` accumulation series + trajectory sentence + the engine's six-component reason tree) → load (acute ÷ chronic, its chart, the range actually held) → what you did (sessions vs the athlete's own average) → the carried-over Pro recovery-vs-load, check-ins, insights and PRs. The HRV and sleep glance charts LEFT — they re-plotted readings Today prints, which now have detail screens behind their own cells. Claim rails: it describes accumulation and trajectory, never injury risk, never a forecast, never a prescription. v6.3 area = **load** (the hero is a load reading); the recovery-vs-load section declares recovery on itself. **Meaning layer (UAT round 2 · U16):** one plain-language reading under each hero in the working voice, composed zone × trajectory (`trends.meaning.*`, en + zh-Hans), plus a collapsed "How to read this" `DetailDisclosureList` per section (scale, cut-points, what moves it, a gloss per fatigue component); a fence test keeps every meaning string free of injury/risk/forecast/prescription wording. **Round 3 (U20):** the "How to read this" lists LEFT the page — the Fatigue card and the Load hero are doors (`TrendDestination.fatigue(range:)` / `.load(range:)`) to `FatigueDetailScreen` / `LoadDetailScreen` in `TrendDetailScreens.swift`, where each reason-tree component carries its weight and gloss and the explainers live; the one-line readings stay on Trends.
- Location: `WorkloadApp/Views/WorkoutLog/WorkoutLogView.swift` + ActiveWorkoutSheet
- Responsibilities: Capture + history (v1.7.3 feature 6): calendar spine (`ScheduleWeekSection`, editable week + day sheets), one program door (`ProgramImportSheet`), session history, new session creation, WorkoutPipeline on save. The verdict card moved to Today. **UAT round 1 · U7/U8 — ONE DOOR:** the header carries a single action, the mic (`LogCaptureSheet`); the "+"-menu trio (Plan Today · My Programs · Import AI) and the "+" template button are RETIRED. The program door is the week strip's own header (it already prints name + position); template AUTHORING moved to the last row of My Programs (`TemplateListView`); with no active program the page shows one hero plate (`BringYourProgramSection`) whose quiet second line opens `TemplatePickerSheet`. **Program start rule (UAT round 2 · U15):** `ProgramScheduleService.defaultAnchor` starts an imported block on the NEXT Monday unless the import day is a Monday; the import sheet's done step offers "Starts next Monday" / "Starts today"; week 1 is materialized whole from its own Monday, never amputated; a position move re-anchors on the CURRENT week's Monday. TODAY's planned day in the week sheet carries "Start this session" (U17) — the second door to `ActiveWorkoutSheet(resolvedPlan:)` beside Today's verdict card. The weekly-review notification (U12) carries `userInfo.route = weeklySummary`; `NotificationRouteDelegate` → `TabRouter.pendingAnchor` → Today scrolls to `WeeklySummaryCard`. **Round 3:** `ProgramOverviewView` day rows push `ProgramDayDetailView` (group → exercise → folded set lines `5 × 5 @ 100 KG` / `3 × 3 @ 80 / 85 / 90 KG` / `@ RPE 8` / `2 RIR` / `BW`, all through `WeightFormatter`), the weeks card has Expand all, its keys decode the page (`TONNAGE · RELATIVE`, `VOLUME VS PLANNED · TRAINED DAYS`, `■ LOGGED · □ PLANNED`, `POSITION · TAP TO MOVE`); `WorkoutSession.volumeIsDistance` (a distance set and no strength volume) routes every work reading through `SessionWorkReading` — tonnage in the athlete's unit, distance as KM/MI, else `LOAD n AU`; never kg on a walk. Now unreferenced and awaiting the audit lane: `TemplateCarouselSection`, `TemplatePreviewSheet`, `PlanTodaySheet`, `TimeRangeSegmentedControl`, `HRVTrendChart`, `SleepTrendChart`.
## Error Handling
