...
- Responsibilities: The "Today" surface (v1.7.3 feature 7 slice 2): check-in prompt → fatigue caution → the day's proposal (`TodayProposalSection` hosts the verdict card + decision seam; no-plan state offers Bring your program / Start unplanned) → readiness hero as evidence → load/metrics/summary/recent sessions. The three body-signal cells in `MetricsStrip` are TAP-THROUGHS (UAT round 1 · U9) — each pushes its own detail screen via `TrendDestination` (`.hrv` / `.rhr` / `.sleep`), which is what let Trends stop re-plotting the same three lines.
