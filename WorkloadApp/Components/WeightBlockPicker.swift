...
struct WeightBlockPicker: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Spacing.sm) {
            // Live readout above the cluster: a debossed readout well (v4.2). Fixed-width
            // (D13(c)) reserved for the widest realistic reading ("999.5") so the numeral never
            // jitters the unit as digits change; the swap rolls via digitRoll (D13(b)).
            ReadoutWell(
                value: centerDisplay.map(numeralString) ?? "—",
                unit: unitLabel,
                widthTemplate: "999.5",
                color: isUnset ? ColorTokens.text2 : ColorTokens.text1
            )
            .fixedSize()
