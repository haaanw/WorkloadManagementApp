...
| Micro / cap   | 12pt | 400    | `micro`                | ≈0.9pt tracking, caps, Latin only. Raised 11→12pt (v6.2, HAN 2026-07-30): the smallest working-voice text (section heads) read too small on device; 12pt stays a step below `smallLabel` |
...
| **Annotation sm** | **11pt** | 400 | **`annoSmall`**    | **Fragment Mono, uppercase, +0.05em — axis labels, timestamps.** Raised 10→11pt (v6.1, HAN 2026-07-30): mono uppercase is the densest text in the app and 10pt read as too small on device. 11pt keeps the ≤12pt cap and stays one step below `anno` so the two sizes remain distinguishable |
...
| **2026-07-30** | **v6.2 — `micro` 11pt → 12pt (HAN)** | HAN's second on-device readability call: the tracked-caps micro labels (the smallest working-voice text; they draw the HRV TREND / LOAD TREND section heads) read too small for general users. Raised to 12pt — still a step below `smallLabel` (13pt), so the ramp's steps survive. Body stays 17pt (the iOS standard). The systemic answer to "text feels small" is **Dynamic Type adoption** (the app currently ignores the system text-size setting entirely) — scheduled as v1.7 Wave 3 work, not solved by inflating fixed sizes. |
