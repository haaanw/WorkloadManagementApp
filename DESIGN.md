...
- **Typeface:** DM Sans — geometric, neutral, precise. Clean terminals, excellent numerics, available as open-source OTF for iOS bundling. Load via Google Fonts for web; bundle as `.otf` in the Xcode project for native.
- **Weights used:** Regular (400) and Medium (500) only. No other weights. No italic.
- **Hero score:** DM Sans Regular, 56pt (or 64pt on larger devices), tabular numerals. Color: Accent only. This is the single colored text element in the UI.
- **Page title:** DM Sans Regular, 28pt, `--text-1`
- **Section header:** DM Sans Medium, 17pt, `--text-1`
- **Body / metric labels:** DM Sans Regular, 15pt, `--text-1` or `--text-2`
- **Secondary info:** DM Sans Regular, 13pt, `--text-2`
- **Micro / caps:** DM Sans Regular, 11pt, `--text-3`, letter-spacing +0.08em, all-caps
...
Bundle `DMSans-Regular.otf` and `DMSans-Medium.otf` in the Xcode project. Register via `Info.plist` → `UIAppFonts`. Reference in SwiftUI: `Font.custom("DMSans-Regular", size: 15)`.
...
8. **Typography uses `Font.custom("DMSans-Regular", size:)` and `Font.custom("DMSans-Medium", size:)`** for all text. Do not use `.system()` or `.headline` / `.body` semantic styles — these override the design system.
...
| 2026-03-21 | DM Sans Regular + Medium only, no other weights  | Hierarchy through size alone — "less is more" applied to typography                 |
