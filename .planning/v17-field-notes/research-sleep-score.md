...
| H-33 | S3 scope cut: `priorWakeZ`, `napMinutes`, and `priorDayLoadZ`/`priorDayActiveEnergyZ` are accepted pass-throughs but never wired in S2 — HIGH_STRAIN_DAY, NAP_DAY, and HIGH_PRESSURE's z-arm CANNOT fire in the S2 shadow | JUDGMENT | S2 scope decision (status-s2 ruling 6): the z's need 28-night buffers and fetches the carrier does not hold this phase | Process guard, not a measurement: the shadow analysis must not read those profiles' absence (or HIGH_PRESSURE's rarity — its hours-arm still fires) as evidence about their triggers; S3 wires the inputs before any profile-frequency conclusion is drawn |
