...
| H-19 | The interim need estimator — p75 of a trailing ≤90-**night** TST buffer, gated at 28 nights — approximates §4's estimator well enough for the shadow run (the unconstrained-night filter, the "trailing 90 *days*" day-indexing, and response-based estimator (b) are deferred to Phase S3) | HYPOTHESIS | None — S2 scope decision; §4's estimator (a) uses p75 for the same robustness reason, and constrained nights populate below p75, limiting their drag | Shadow: when S3 adds the unconstrained filter, recompute both estimates over the same nights; divergence > the 15-min deadband on real data means the interim estimator was steering need and its nights must be re-scored in the dual-run analysis |
| H-20 | `isSourceStable` reads false for the source-change night plus the following 14 nights | HYPOTHESIS | None — §5.2 names source stability but no window; 14 matches H-15's own kill-test window so the two rows measure the same thing | Falls out of H-15's test: if scores in the 14-night window correlate with readiness as well as matched stable nights, the window (and H-15's multiplier) shrink toward zero |
| H-21 | Stage EWMAs run at the sleep signal's 7-day half-life (`BaselineConstants.sleepHalfLifeDays`) and carry no scoring authority until ≥7 same-source folds | HYPOTHESIS | None — §5.1 writes "EWMA(deep min, same source)" with no constant; §9.5 ruling 3 says an unconverged EWMA drops the component but names no bound. 7 folds mirrors the tier gate (`minNightsForV2`) | Shadow: distribution of q = tonight/EWMA in nights 7–14 vs nights 30+. If early-window q dispersion is materially wider, raise the fold minimum; if indistinguishable, lower it |
| H-22 | A prior-wake span outside (0 h, 48 h] is a data gap, not a vigil — it reads as missing and fires no HIGH_PRESSURE trigger | HYPOTHESIS | None — §9.2 defines `priorWakeHours` but not its failure modes; 48 h is a sanity bound (a true >48 h vigil in an athlete log is overwhelmingly a missing night) | Shadow: count nights with a computed span > 24 h alongside their HealthKit gaps. If genuine long-wake nights (data present, no sleep) appear, the bound moves up and those nights re-enter the trigger |
| H-23 | Midpoint SD-14 and deviation require ≥5 nights in the buffer (below it both read nil and no timing trigger fires) | HYPOTHESIS | None — §9.2 names the 14-night window but no minimum; 5 mirrors `BaselineEngine.madMinValid` (same "no fabricated dispersion" reasoning) | Shadow: regularity-component availability curve over the first fortnight. If nights 5–9 produce SD values that whipsaw the regularity score (>15-pt night-to-night swings), the minimum rises |
| H-24 | The continuity opportunity window is the dominant source's explicit `inBed` samples when present, else the span of its asleep+awake samples; cross-source `inBed` is never borrowed | HYPOTHESIS | §3 (writers differ; Health may re-bin intervals) motivates same-source discipline, but no spec section defines the fallback window | Shadow: on nights where both an explicit same-source `inBed` and a span exist, compare the two efficiencies. Systematic divergence > ~3 efficiency points means the span fallback biases continuity and Tier A should require explicit `inBed` |
| H-25 | A gap > 90 min between the dominant source's consecutive asleep samples starts a new sleep session; the MOST RECENT session is last night (a gap of exactly 90 still bridges) | HYPOTHESIS | None — no spec section clusters the fetch window; without it, the up-to-~32 h window (run at 08:00) merges two post-midnight nights and naps into one Night with TST ≈ 900 and a garbage midpoint | Shadow: log per-night max internal gap and session count. Genuine single nights split by a > 90-min waking (truncated TST) → raise the gap; naps within 90 min of the main session merging in → lower it |
| H-26 | A session that ended less than 120 min before the pipeline run reads as possibly-ongoing and does not fold; the complete night folds on a later run the same day | HYPOTHESIS | None — a 02:00 mid-night open or a mid-sync truncation is indistinguishable from a finished short night at fetch time; 120 min is a chosen hold | Shadow: count skipped-then-refolded nights and their TST deltas. Full nights routinely delayed a whole day (the day's only run lands inside the hold) → shorten; truncated nights still folding → lengthen |
| H-27 | At gate-open the first learned need is the bounded p75 estimate itself (snap), not a ±10-min/week ramp from 7.5 h | JUDGMENT | §4's deadband/hysteresis are defined against "the current stored need" and none exists at gate-open; ramping would leave a 9 h sleeper mis-targeted for months (S2 ruling 1, status-s2) | Shadow: on gate-open nights, compare the snap against the ramp trajectory vs next-day sleep-free readiness for athletes whose estimate sits far from 7.5 h. If the weekly updates immediately walk the snap back by more than the deadband, the snap overshot and the ramp (or a capped snap) wins |
| H-28 | The §4 source-change "freeze need" is implemented as the cleared TST buffer re-closing the 28-night gate (stored need keeps serving, un-updated, natural thaw); the persisted `sleepV2NeedFrozen` flag tracks ONLY the chronic freeze | JUDGMENT | §4 names the freeze but no mechanism or thaw; the cleared gate gives both without a second state flag (S2 ruling 2, status-s2; SPEC-8 ruling: document as chronic-only, no rename — migration safety) | Shadow: after a real device switch, compare the served (frozen) need against the re-learned need at re-gate. First-recompute divergence > the 15-min deadband means stale need mis-scored the window — add an explicit freeze flag and a faster thaw |
| H-29 | `daysSinceRhythmBreak` counts INCLUDING tonight (a first shifted night reads 0; the carried counter is as-of-last-fold and reads +1) | JUDGMENT | §9.2 defines the field but not the counting convention; ACUTE_SHIFT (`≤ 2`) must be able to fire on the break night itself — the state it exists to catch (S2 ruling 3, status-s2) | Shadow: ACUTE_SHIFT firing one night late, or re-firing on the night after a single break, means the convention (or the +1 carry) is wrong |
| H-30 | The midpoint is a signed scalar — minutes relative to the WAKE-day midnight, no modular wrap at 00:00 | JUDGMENT | §9.2 names midpointSD14/deviation but no representation; a wrapped (mod-1440) scalar makes a 23:50 → 00:10 drift read as ~1420 min and destroys SD/median continuity (S2 ruling 4, status-s2) | Shadow: SD14 continuity across athletes whose midpoints straddle midnight. Whipsawing SD on stable straddle-sleepers would indict the representation; by construction the signed form cannot wrap, so this row retires when straddle stretches show smooth SD |
| H-31 | Prequential SD split: `makeInput` scores tonight against the PRE-fold buffer (t−1); the per-night irregularity flag and the chronic-exit counter use the POST-push SD (the fortnight ENDING tonight) | JUDGMENT | §9.3's "that night's SD14" reads post-inclusive while the ACUTE_SHIFT trigger's "prior midpointSD14" reads pre-fold; the split honors both and matches `BaselineEngine.score`/`step` (S2 ruling 5, status-s2) | Shadow: recompute CHRONIC_IRREGULAR entries/exits under the opposite split on the same nights. Any state flip that differs goes to HAN to adjudicate which reading of §9.3 stands |
| H-32 | Need updates are due when ≥ 7 days have passed since the last update (rolling week), not on §4's literal "same weekday" | JUDGMENT | A same-weekday schedule needs a stored anchor weekday and stalls a full week when the anchor day is missed; the rolling week keeps the "weekly, never nightly" intent (S2 implementation choice) | Shadow: log update timestamps. If weekday drift changes any need trajectory by more than the 15-min deadband vs a strict same-weekday replay, implement the calendar-anchored cadence |
| H-33 | S3 scope cut: `priorWakeZ`, `napMinutes`, and `priorDayLoadZ`/`priorDayActiveEnergyZ` are accepted pass-throughs but never wired in S2 — HIGH_STRAIN_DAY, NAP_DAY, and HIGH_PRESSURE's z-arm CANNOT fire in the S2 shadow | JUDGMENT — **CLOSED by S3 (2026-08-03)**: all four pass-throughs wired (H-34 priorWakeZ, H-35 naps, H-36 load z, H-37 energy z) | S2 scope decision (status-s2 ruling 6): the z's need 28-night buffers and fetches the carrier does not hold this phase | The process guard survives the closure in dated form: nights folded BEFORE the S3 wiring landed still carry absent pass-throughs, so profile-frequency reads segment on the fold date; nights after it are full-signal. **Boundary persisted (S3 review round):** every `SleepShadowNight` row stamps `schemaVersion` (2 = the S3 wiring) at creation, so the segmentation reads a stored field, never a date reconstructed after the fact; a future wiring change bumps the default |
| H-34 | `priorWakeZ` is a robust z of tonight's H-22-valid span against the carried 28-night buffer — `(x − median) / (madScaleK × MAD)` — requiring ≥ 5 buffered spans; a zero MAD reads nil (the HIGH_PRESSURE z-arm never fires on it; the hours-arm still can) | JUDGMENT | §9.2 names the 28-night MEDIAN but no scale and no minimum; the MAD × 1.4826 scale is `BaselineEngine.madScaleK`'s own robust convention and the 5-night floor mirrors `madMinValid` (H-23's no-fabricated-dispersion reasoning) | Shadow: distribution of priorWakeZ on nights the hours-arm also fired. The z-arm firing on ordinary nights for hyper-regular sleepers (MAD too tight) or never firing for anyone (floor too high) moves the scale or the minimum. **Buffer-push staleness (S3 review round):** a span longer than H-41's 24 h reads as a fold-gap artifact (a missed-fold day, not a vigil — H-22's spirit) and never enters the buffer, even though it sits inside H-22's 48 h scoring bound — one ~39.5 h push would drag the 28-night median/MAD for a month |
| H-35 | Nap selection: the fetch window's NON-main H-25 session clusters count as naps only when each carries ≥ 20 unioned asleep minutes (§9.3's own NAP_DAY trigger) AND lies strictly between the previous main sleep's end and tonight's session start; with no known previous sleep end, `napMinutes` reads 0 (absent) | JUDGMENT | §9.2 writes "daytime `asleep` samples since last main sleep" but no session rule; the H-25 clusters are reused rather than inventing a second clustering, and an unknown previous sleep end makes a candidate indistinguishable from an unfolded previous night | Shadow: log candidate counts/minutes vs credited nap minutes. Naps merging into the main session point at H-25's gap, not this rule; genuine naps discarded because the previous night never folded indict the lastSleepEnd guard |
| H-36 | `priorDayLoadZ` is a plain sample z of the prior day's session-TSS sum against the trailing 28 calendar days before it, ZERO-FILLED (a rest day is a genuine 0-load observation), computable only when observed history COVERS the whole window (earliest fetched session ≤ window start; the pipeline fetch is bounded at 40 days) and variance > 0 | JUDGMENT | §9.2 writes "session TSS … vs 28-day mean" and no more; zero-filling is what makes the reference a daily-load distribution instead of a per-session one, and the coverage gate stops z-scoring against fabricated zeros from pre-history days | Shadow: HIGH_STRAIN_DAY (z ≥ +1) firing rate vs training frequency. Firing on most ordinary training days of low-frequency athletes (zeros dominating the SD) switches the reference to nonzero days; H-03's kill test still decides whether the strain channel lives at all |
| H-37 | `priorDayActiveEnergyZ` is a plain sample z of the prior day's HealthKit active-energy daily sum against the PRESENT days of the trailing 28 — ≥ 21 present days required, missing days never zero-filled, a missing prior day reads nil | JUDGMENT | §9.2 names active energy vs the 28-day mean; a wear gap is missing data, not a rest day, so the present-day gate replaces zero-filling | Shadow: present-day counts per window. The ≥ 21 gate silencing most real windows lowers it; watch-off days sneaking in as near-zero sums add a floor filter |
| H-38 | The sleep-free readiness proxy = `RecoveryScoreEngine.compute` **baseScore** with the sleep component omitted (weights renormalize over HRV/RHR/wellness — the engine's documented missing-data path), the trend modifier zeroed (`recentScores: []`, because trend history contains sleep and would leak the tested signal into the outcome), and the wellness arm **rebuilt sleep-free** (see REVISED); stored nil when no component scored — never the engine's neutral 50 | **REVISED (S3 adversarial review, 2026-08-03)** — the first cut fed the composite `WellnessCheckIn.wellnessScore` into the proxy, and that composite is 25% `sleepQuality`: a subjective rating of the tested night was ~8.3% (full-input) to 25% (wellness-only) of the "sleep-free" outcome, biasing criterion 1 anti-conservatively. Fix: the proxy's wellness arm is reconstructed from the raw sub-ratings (soreness + energy + stress rescaled to 0–100) when the check-in row is available for the wake day (in the morning join it always is); were only a composite available, wellness would be EXCLUDED (HRV + RHR only); nil when nothing remains. The stored `nextMorningWellness` audit field keeps the full composite. Kill test: `test_morningJoin_proxyIdentical_whenOnlySleepQualityDiffers` (sleepQuality 1 vs 5, all else equal ⇒ identical proxies) | §6 names "the sleep-free readiness proxy" without a formula, and no existing service computes one (`PRSReadinessInputBuilder` recomputes FULL readiness including a sleep z), so the least-invented derivation is the live engine minus its sleep term — minus, post-revision, every sleep-rating input | Criterion 1 runs on this outcome. If HRV/RHR baselines are absent on many mornings the proxy collapses toward (sleep-free) wellness-only — the per-component availability is reported beside criterion 1 before any verdict is trusted |
| H-39 | Join cadence: the morning join fills a night's row only after its WAKE DAY has elapsed, from that day's PERSISTED `RecoverySnapshot`/`WellnessCheckIn` (the D-03 target-day discipline); the evening join waits one further day so the write-once next-day felt-right window has closed; both stamp their timestamp even when every joined value is nil (absence is the record, and a data-less day is not re-scanned forever) | JUDGMENT | §6 lists the join fields but not when or from where they are read; joining at fold time would capture a nil wellness (entered later that day) and "today's" live HRV belongs to the FOLLOWING night's row | Shadow: count rows whose morning join found no snapshot (app not opened that day). A miss rate that starves criterion 1's n (> ~25% of folded nights) argues for a same-day provisional join upgraded at day close |
| H-40 | Analysis gates: split-half needs ≥ 14 nights per half; a stage residual SD needs ≥ 30 within-source consecutive-night q pairs; a source needs ≥ 7 nights before its stage coverage is judged; the stage-curve q-range constant (0.90) MIRRORS the engine's private anchor domain (0.40 … 1.30) by value; and **per-criterion bootstrap block lengths (S3 review round): criterion 3 resamples with 14-night blocks** — its predictor `midpointSD14` is a 14-night rolling SD (lag-1 autocorrelation ≈ 0.93), and a block shorter than the predictor's dependence length narrows the CI anti-conservatively — while criterion 1 keeps the 7-night default | JUDGMENT | §6 names none of these; the numbers echo existing gates (7 = `minNightsForV2`) and the mirror constant exists because the engine's anchor table is deliberately private | The gates are analysis-side parameters (overridable per run, reported with every verdict). The mirror constant moves whenever H-11's own kill test moves the stage anchors — this row is the standing reminder that `SleepShadowAnalysis.stageCurveQRange` moves with them. Registered deviations (S3 review round): criterion 1 excludes Tiers D/E from §6's "≥ 60 nights" (Tier D is v1-identical by contract and Tier E scoreless — both arms coincide there and only dilute Δρ), a deliberate deviation from the section's letter. The bootstrap CI reads nil when fewer than half the resamples produce a defined statistic (a constant column makes Spearman undefined) — an approximation registered so a degenerate-heavy series degrades to `insufficientData` rather than a CI built on a biased subset. **⚠ FOR HAN:** criterion 2's bar as written has near-zero power — a residual SD must exceed the FULL 0.90 q-range to fail, and nightly q typically wobbles within ±0.2, so the criterion can practically never fire; §6's bar may need tightening (e.g. a fraction of the range). That is HAN's call, not the analysis code's |
| H-41 | The carried `lastSleepEndDate` is STALE when it lies more than 24 h before tonight's session start (a missed-fold day sits in between). A stale end zeroes the nap sum (`napMinutes` reads 0 = unknown — the "candidates" in the widened gap are the unfolded previous night, which would otherwise read as a ~400-min nap and fire NAP_DAY falsely) and blocks the H-34 prior-wake buffer push (the ~39.5 h span is a fold-gap artifact, not a vigil — H-22's gap-not-vigil spirit). The `makeInput` scoring read keeps H-22's own 48 h bound | JUDGMENT (S3 adversarial review, 2026-08-03) | No spec value; 24 h is the tightest bound that always spans one true sleep-wake cycle — any real same-cycle gap between the previous main sleep's end and tonight's start fits inside it, while any missed-fold day cannot | Kill test: `test_missedFoldDay_staleLastSleepEnd_noFalseNap_noPriorWakeBufferPush` (the Mon-fold → Tue-skip → Wed-fold walk: napMinutes == 0, buffer unchanged). Shadow: count nights with a stale end alongside genuine >24 h wake stretches (data present, no sleep); if real vigils appear stale-gated, the bound moves up and re-splits from H-22 |
