...
- [x] Final review gate COMPLETE (2026-07-13): full unit suite green; codex cross-lane review found 6 issues -> 5 fixed in `045a944` (picker hydration desync, SwiftData-across-await, LLM downgrade guard, onSaved callback, stagger cap/O(n^2)); 6th (isPro) resolved by product decision = custom-exercise persistence FREE for all tiers. ShareImportSheet left dormant (user decision). Repo moved to ~/dev/Tonus (out of iCloud). Round CLOSED: 1cb37cd, 4af9276, ebdbdcb, 2f94d25, 045a944 all pushed.
