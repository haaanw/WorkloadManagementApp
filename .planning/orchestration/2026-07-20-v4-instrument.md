...
- [x] Stage 2″ dispatched + verified + committed (2026-07-20): Home hero + Load ACWR onto panels with TickScales, verdict header on dial numerals + KeyRow decisions (guard tests green), Recovery mono, FULL accent sweep — `ColorTokens.accent` now only in the dead AppShell + token definition; red renders solely from TickScale needles + InkTabBar tick. Sheets/auth keys pass. Zero xcstrings churn. Gate 783/781/0/2 incl. guard + UI tests. Deferral flagged: system sheet nav chrome (glass Cancel pill + system-font titles) still stock — orchestrator judgment for 4″.
- [ ] Stage 3″ dispatched (motion pass — brief: `stage-v4-3-brief.md`)
