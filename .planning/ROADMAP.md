...
Plans:
- [ ] 27-01-PLAN.md — Pure `StrengthLoadEngine` (reuses `SetRecord.estimated1RM`/`rir`/`rpe`; relative-intensity buckets, hard-set rule, per-muscle hard sets + acute/chronic elevation + same-region recurrence) + optional local-only `StrengthLoadState`
- [ ] 27-02-PLAN.md — Pure `LoadDistributionEngine` (unified daily-load series + completeness-gated Foster monotony/strain with streak/density/spike fallback)
- [ ] 27-03-PLAN.md — Pure `StrainRiskEngine` fixed glass-box fusion (score + `StrainRiskZone` + ranked factors + confidence; reuses FatigueIndexEngine/NiggleInjuryDeriver/BaselineEngine) + isolation + tier-fence + full-suite + no-prediction-copy guards
